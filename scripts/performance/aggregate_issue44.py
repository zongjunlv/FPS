...
METRICS = (
    ...
    "averageMainThreadMs", "p95MainThreadMs", "averageGcBytesPerFrame",
    "perceptionChecks", "maximumPerceptionLatencyFrames",
    "maximumSightResultDelayFrames", "sightScheduledBatchCount",
    "sightCompletedBatchCount", "sightScheduledCommandCount",
    "sightDiscardedStaleResultCount", "sightPeakBatchSize",
...
CONFIG_FIELDS = (
    ...
    "behaviorProfile", "aiLodEnabled", "spatialIndexEnabled",
    "perceptionChecksPerFrame", "enemyCount", "warmupSeconds",
    "configuredSampleSeconds", "seed",
)
...
def main():
    ...
    for path in args.reports:
        ...
        groups[bool(report["jobSightEnabled"])].append(report)

    for enabled, reports in groups.items():
        if len(reports) != args.expected_rounds:
            label = "job" if enabled else "sync"
            raise ValueError(
    ...
    result = {
        ...
        "sync": {field: median(groups[False], field) for field in METRICS},
        "job": {field: median(groups[True], field) for field in METRICS},
    }
    result["changePercent"] = {
        field: ((result["job"][field] - result["sync"][field]) /
                result["sync"][field] * 100.0)
        if result["sync"][field] else 0.0
    ...
    args.output_json.write_text(
        ...
        encoding="utf-8")
    lines = [
        "# Issue 44：同步射线与 Job/Burst 批量视线 A/B", "",
