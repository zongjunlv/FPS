...
玩家击杀自动装填使用统一战斗事件上下文：权威击杀经过玩家归属校验和生成编号去重后，为当前弹匣补充3发；满弹、切枪中和效果移除均有明确边界，弹药变化会立即同步 HUD。

玩家生命严格低于35%时会通过条件型 Effect 获得35%射速加成，恢复到阈值后自动撤销；效果与射速卡牌使用同一聚合规则，并同步实际开火间隔、武器动画和 HUD 总倍率。
